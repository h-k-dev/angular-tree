...
accessor functions, and apply the tree's *intents* (`moved`, `renamed`, …) to
...
| `pTemplate="type"` per node type | `*treeNodeDef="…; when: isType"` — `when` is a *type guard*, the template context narrows |
...
| `[draggableNodes]`/`[droppableNodes]` + `(onNodeDrop)` | always draggable unless `disableDrag`; validate with `disableDrop(ctx)` *before* the drop lands; apply `(moved)` yourself |
...
key is `null` — are synthesized in *your* data layer with stable keys. The
...
interpreted by *your* `moved` handler:
