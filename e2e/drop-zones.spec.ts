...
test.describe('pointer drop zones', () => {
  ...
  const dragToZone = async (
    ...
    await page.mouse.move(to.x + to.width / 2, to.y + to.height * fraction, {
      steps: 12,
    });
    await page.mouse.up();
  };

  test("'after' an expanded folder inserts as its FIRST CHILD, not after the subtree", async ({
    page,
  }) => {
    ...
    await expect(page.locator('.app-last-intent')).toContainText(
      'moved 1 node(s) → cases/0/0/0@0',
    );
  });

  test("'after' a collapsed folder keeps sibling semantics", async ({
    page,
  }) => {
    ...
    await expect(page.locator('.app-last-intent')).toContainText(
      'moved 1 node(s) → cases/0/0/0@1',
    );
  });

  test("'before' a folder inserts as its preceding sibling", async ({
    page,
  }) => {
