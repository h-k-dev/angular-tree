...
- ✅ **`labelOverflow` input — truncation actually works under virtualization (settled 2026-07-31, user call — genesis: a consumer picker followed docs/VIRTUALIZATION.md's "truncate long names" advice and still got a horizontal scrollbar)** — CDK's `.cdk-virtual-scroll-content-wrapper` is absolutely positioned with only `min-width: 100%` (a floor, not a ceiling), so it shrink-wraps to the widest nowrap row: labels never meet an edge and `text-overflow: ellipsis` silently never engages. The wrapper lives in the CDK component's own template — unreachable from scoped styles — so `labelOverflow: 'scroll' | 'ellipsis'` (default `'scroll'` = prior behavior) fixes it with NO `::ng-deep` and no DOM reach-in: `'ellipsis'` makes `.tree-viewport` an inline-size query container and caps `.tree-node` at `100cqw` — cqw resolves against the viewport, skipping the circular shrink-wrap parent (row `max-inline-size: 100%` can't work: a percentage against a shrink-to-fit parent is circular), so capped rows stop the wrapper growing and consumer ellipsis engages. Horizontal scrolling is deliberately gone in that mode; deep trees stay on the `'scroll'` default. Label CSS itself (`overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-inline-size: 0`) stays consumer-side per no-UI-we-don't-own. docs/VIRTUALIZATION.md amended (the old advice was incomplete as written) incl. the global-stylesheet recipe for consumers styling from outside (plain global CSS reaches the wrapper; `::ng-deep` was never needed there). Demo: the Media library seeds an extremely long video title and runs `'ellipsis'`; `e2e/label-overflow.spec.ts` pins the geometry (wrapper = viewport width, zero horizontal overflow, the long label genuinely clipped). Middle-ellipsis (macOS-style) settled same day as post-v1 — zero-dep directive, chenglou/pretext rejected (ROADMAP2 Phase 13, decision 13)
...
- **`labelOverflow: 'scroll' | 'ellipsis'` input (settled 2026-07-31, user call)** — under virtualization, CDK's absolutely-positioned content wrapper shrink-wraps to the widest nowrap row (its `min-width: 100%` is a floor, not a ceiling), so the consumer's `text-overflow: ellipsis` silently never engages — following the docs produced a horizontal scrollbar instead. The tree owns the geometry fix (the wrapper is unreachable from consumer scoped styles): `'ellipsis'` makes the viewport an inline-size query container and caps rows at `100cqw` — no `::ng-deep`, no reach into CDK DOM. Default `'scroll'` preserves horizontal scrolling (deep trees); the label's ellipsis CSS stays consumer-side (no UI we don't own). Middle-ellipsis (macOS Finder style) is post-v1: a zero-dep directive (canvas `measureText` + `Intl.Segmenter` binary search); chenglou/pretext rejected — multiline layout engine with no middle-truncation, and even an optional peer dep would dilute "CDK as the only runtime dependency" (ROADMAP2 decision 13)
- **`--tree-row-inset` token — one-off leading inset shared by rows, guides and the drop indicator (settled 2026-08-03, consumer report)** — a consumer padding their own row-content wrapper (`padding-inline-start` on def content) shifts the toggle column but NOT the indent guides: rows and guides are siblings under the CDK content wrapper sharing one origin, so the guide line stays at `level·indent + indent/2` while the toggle centre moves by the content padding `P` — a constant, depth-INDEPENDENT error of `P + (slot − indent)/2` that silently breaks the documented thread-line recipe (`--tree-indent: var(--tree-toggle-size)` assumed zero content inset; now stated as its precondition in THEMING.md). The only prior lever, `--tree-indent: slot + 2P`, pays `2P` per LEVEL to cancel a one-off `P` (depth 5: 180px instead of 100px). Fix: all three `level · indent` sites (row `padding-inline-start`, guide `inset-inline-start`, drop-indicator `inset-inline-start` — verified the complete set; drag hit-testing emits a `level` and positions via the same CSS) add `var(--tree-row-inset, 0px)`. They MUST move together — an unshifted drop indicator would point `P` left of the row content it names. Fully backwards-compatible (`0px` default); elbow geometry verified self-consistent (rows and guides shift uniformly, so the └ still lands at the shifted child column's start, `O + (L+1)·indent + P`). Viewport-padding alternative rejected: horizontal padding on a virtual-scroll viewport entangles scroll geometry and the `labelOverflow` container-query math. Knock-on documented in THEMING.md: full-bleed row overlays walking back by `-level·indent` must also subtract the inset. Demo: the VS Code Explorer runs `--tree-row-inset: 8px` (real VS Code insets Explorer rows from the sidebar edge — the example gets more authentic, and its guides prove the alignment survives); `e2e/indent-guides.spec.ts` pins guide-under-toggle WITH the inset at two depths plus the RTL mirror
