...
Status: #1–#4 shipped 2026-07-12 (incl. the same-day `SelectionModel` removal and the `expandedKeys()` method removal; All-In-1 migrated to both controlled bindings, Static shows live `[(expandedKeys)]` status bars + the `SelectEvent` inspector strip, Lazy Load Only gained the `[childrenDeps]` ref switcher); 5–6 pending.
